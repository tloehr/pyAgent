...
NET_STATUS: json = {"led_all": "off", "wht": "netstatus"}
...
class Agent:
    ...
    def __init_mqtt(self):
        ...
        mqtt_broker: str = ""
        self.__mqtt_client = mqtt.Client()
        self.__mqtt_client.max_inflight_messages_set(self.__my_context.configs["mqtt"]["max_inflight"])
        self.__mqtt_client._clean_session = self.__my_context.configs["mqtt"]["clean_session"]

        while not self.__mqtt_client.is_connected():
            mqtt_broker = self.__my_context.configs["mqtt"]["broker"][current_broker]
            try:
                ...
                current_broker += 1
                if current_broker >= len(self.__my_context.configs["mqtt"]["broker"]):
                    current_broker = 0
    ...
    def on_message(self, my_client, userdata, msg):
        ...
        try:
            ...
            match cmd:
                ...
                case "reset_status":
                    self.__my_status_job.reset_stats()
                case "status":
                    self.__my_status_job.send_status()
                case _:
